# -*- coding: utf-8 -*-
"""
Created on Wed Feb 24 16:59:30 2021
...
class GradCam():
    ...
    def from_interp(cls,learn,interp,img_idx,ds_type=DatasetType.Valid,include_label=False):
        ...
        xb_img = PIL.Image(interp.data.denorm(xb)[0])
    ...
    def plot(self,filename,plot_hm=True,plot_gbp=True):
        ...
        if hasattr(self, 'label2'):
            label2_title = f' {self.label2} {self.prob2:.3f}'
            if plot_hm:
        ...
        fig.savefig("uploads\\gradcam_"+filename)

        return self.label1,self.prob1
